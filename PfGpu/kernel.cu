...
int main()
{
	...
	for (int data_i(0); data_i < raw_num_times; ++data_i)
	{
		//Sample
		curandGenerateNormal(gen, rnd_x, particle_num, 0.0, sample_sigma);
		curandGenerateNormal(gen, rnd_y, particle_num, 0.0, sample_sigma);
		Sample <<<ceil(particle_num / thread_dim_x), thread_dim_x >>>(p_state, rnd_x, rnd_y, 1, particle_num);

		//Evaluate
		Evaluate  <<<ceil(particle_num / thread_dim_x), thread_dim_x >>>
			(p_state, p_score, d_beaconset, beacon_num,
